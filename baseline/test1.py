...
from utilities.utils import to_cuda_if_available, generate_tsv_wav_durations, meta_path_to_audio_dir, get_durations_df
...
def _load_crnn(state, model_name="model"):
    ...
    crnn = CNN_TRANS(*crnn_args, **crnn_kwargs)
...
if __name__ == '__main__':
    ...
    parser.add_argument("-m", '--model_path', type=str,
                        default='./stored_data/MeanTeacher_with_synthetic/model/ssa_best',
    ...
    parser.add_argument("-s", '--save_predictions_path', type=str, default='./stored_data/MeanTeacher_with_synthetic_47.93/predictions/predict44.tsv',
    ...
    durations_validation = get_durations_df(cfg.eval_desed, cfg.audio_eva_dir)
    evaluation_labels_df = groundtruth.drop("feature_filename", axis=1)

    # Preds with only one value
    single_predictions = get_predictions(params["model"],
                                         params["dataloader"],
                                         params["many_hot_encoder"].decode_strong,
                                         params["pooling_time_ratio"],
                                         median_window=params["median_window"],
                                         median_window_short=4,
                                         median_window_media=14,
                                         median_window_long=39,
                                         save_predictions=f_args.save_predictions_path)
    compute_metrics(single_predictions, evaluation_labels_df, durations_validation)
    ...
    pred_ss_thresh = get_predictions(params["model"],
                                     params["dataloader"],
                                     params["many_hot_encoder"].decode_strong,
                                     params["pooling_time_ratio"],
                                     ...
                                     median_window_long=39,
                                     save_predictions=f_args.save_predictions_path)
    psds = compute_psds_from_operating_points(pred_ss_thresh, evaluation_labels_df, durations_validation)

    psds_score(psds, filename_roc_curves="./figures/psds_roc.png")