    ...
    def gaussian_noise(features, snr):
        """Apply gaussian noise on each point of the data
            ...
            Args:
                ...
                snr: list, average snr to be used for data augmentation
            Returns:
                numpy.ndarray
                Modified features
                """
        ...
        if len(features.shape) == 3:
            ...
            feat_used = features
        snr1 = np.random.uniform(snr[0],snr[1])
        std = np.sqrt(np.mean((feat_used ** 2) * (10 ** (-snr1 / 10)), axis=-2))
        try:
...
class TimeMoveData(Transform):
    def __init__(self, mean=0, time_std=None, apply_to_label=None, max=None):
        self.mean = mean
    ...
    def change_time_move(self):
        self.time_move = np.random.normal(self.mean, self.time_std)
        self.time_move = int(self.time_move)
        while self.time_move < -self.max or self.time_move > self.max:
            self.time_move = np.random.normal(self.mean, self.time_std)
            self.time_move = int(self.time_move)
    ...
    def transform_data(self, data):
        if same == 0:
            self.time_move = self.change_time_move()
        data = np.roll(data, self.time_move, axis=0)
        return data

    def transform_label(self, label):
        if self.apply_to_label:
            label_time_move = int(np.around(self.time_move/4.0))
            label = np.roll(label, label_time_move, axis=0)
            return label
        else:
            return label

class FreqMoveData(Transform):
    def __init__(self, mean=0, std=8/3, max=8):
        self.mean = mean
        self.std = std
        self.freq_move = 10
        self.max = max
    def transform_data(self, data):
        while self.freq_move < -self.max or self.freq_move > self.max:
            self.freq_move = np.random.normal(self.mean, self.std)
            self.freq_move = int(self.freq_move)
        data = np.roll(data, self.freq_move, axis=1)
        return data
...
def get_transforms(frames=None, scaler=None, add_axis=0, noise_dict_params=None, combine_channels_args=None, time_move=None,
                   freq_move=None):
    transf = []
    unsqueeze_axis = None
    ...
    if time_move is not None:
        transf.append(TimeMoveData(**time_move))

    if freq_move is not None:
        transf.append(FreqMoveData(**freq_move))
