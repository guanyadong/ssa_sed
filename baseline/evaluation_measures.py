...
def get_predictions(model, dataloader, decoder, pooling_time_ratio=1, thresholds=[0.5], median_window=1,
                    median_window_short=1,
                    median_window_media=1,
                    median_window_long=1,
    ...
    for i, ((input_data, _), indexes) in enumerate(dataloader):
        ...
        model, input_data = to_cuda_if_available(model, input_data)
        with torch.no_grad():
            pred_strong, _ = model(input_data, c_epoch=1)
        ...
        for j, pred_strong_it in enumerate(pred_strong):
            for threshold in thresholds:
                ...
                pred_strong_m0 = scipy.ndimage.filters.median_filter(pred_strong_bin[:,0], (median_window_short))
                pred_strong_m1 = scipy.ndimage.filters.median_filter(pred_strong_bin[:,1], (median_window_media))
                pred_strong_m2 = scipy.ndimage.filters.median_filter(pred_strong_bin[:,2], (median_window_media))
                pred_strong_m3 = scipy.ndimage.filters.median_filter(pred_strong_bin[:,3], (median_window_short))
                pred_strong_m4 = scipy.ndimage.filters.median_filter(pred_strong_bin[:,4], (median_window_short))
                pred_strong_m5 = scipy.ndimage.filters.median_filter(pred_strong_bin[:,5], (median_window_long))
                pred_strong_m6 = scipy.ndimage.filters.median_filter(pred_strong_bin[:,6], (median_window_long))
                pred_strong_m7 = scipy.ndimage.filters.median_filter(pred_strong_bin[:,7], (median_window_long))
                pred_strong_m8 = scipy.ndimage.filters.median_filter(pred_strong_bin[:,8], (median_window_media))
                pred_strong_m9 = scipy.ndimage.filters.median_filter(pred_strong_bin[:,9], (median_window_long))
                ...
                pred_strong_mx = pred_strong_m.transpose()
                pred = decoder(pred_strong_mx)
...
def get_predictions1(model, dataloader, decoder, pooling_time_ratio=1, thresholds=[0.5],
...
def get_predictions0(model, dataloader, decoder, pooling_time_ratio=1, thresholds=[0.5],
                     median_window_short=1,
                     median_window_media=1,
                     median_window_long=1,
                    median_window=1, save_predictions=None):
    ...
    for item in model.named_parameters():
        if item[0] == "cnn.cnn.conv0.weight":
            pass
    ...
    for i, ((input_data, _), indexes) in enumerate(dataloader):
        ...
        input_data = to_cuda_if_available(input_data)
        with torch.no_grad():
            pred_strong, _ = model(input_data, 1000)
        ...
        for j, pred_strong_it in enumerate(pred_strong):
            for threshold in thresholds:
                ...
                # pred_strong_mx = scipy.ndimage.filters.median_filter(pred_strong_bin, (median_window, 1))
...
def compute_metrics(predictions, gtruth_df, meta_df=None):
    ...
    if meta_df is not None:
        ...
        return macro_f1_event, events_metric